...
def load_command_table(self, _):
    ...
    if get_vmware_provider() == VmwareProviders.CS:
        ...
        with self.command_group('vmware vm', client_factory=cf_vmware_cs) as g:
            g.custom_command('list-template', 'list_vm_template_by_PC_cs')

        with self.command_group('vmware', client_factory=cf_vmware_cs) as g:
            g.custom_command('list-virtual-networks', 'list_virtual_networks_cs')
            g.custom_command('set-region', 'set_region_cs')
            g.custom_command('get-region', 'get_region_cs')
            g.custom_command('list-resource-pool', 'list_resource_pool_by_PC_cs')

        with self.command_group('vmware', client_factory=cf_private_cloud_by_region) as g:
            g.custom_command('list-private-cloud', 'list_private_cloud_by_region_cs')
