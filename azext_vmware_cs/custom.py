...
def create_vmware_vs_vm():
    return None

def set_provider(provider_name):
    import os
    from knack.config import get_config_parser
    from ._config import (VMWARE_CONFIG_FILE, CONFIG_VMWARE, CONFIG_PROVIDER, VmwareProviders)
    from azure.cli.core._config import GLOBAL_CONFIG_DIR

    config = get_config_parser()
    config.read(VMWARE_CONFIG_FILE)

    if not config.has_section(CONFIG_VMWARE):
        config.add_section(CONFIG_VMWARE)

    config.set(CONFIG_VMWARE, CONFIG_PROVIDER, provider_name)

    if not os.path.isdir(GLOBAL_CONFIG_DIR):
        os.makedirs(GLOBAL_CONFIG_DIR)
    with open(VMWARE_CONFIG_FILE, 'w') as configfile:
...
def list_private_cloud_by_region_cs(client):
    return client.list()


def set_region_cs(client, region_name):
    from knack.config import get_config_parser
...
def list_resource_pool_by_PC_cs(client, private_cloud, resource_pool=None):
...
def list_virtual_networks_cs(client, private_cloud, resource_pool=None, virtual_network=None):
...
def list_vm_template_by_PC_cs(client, private_cloud, resource_pool=None, template=None):
